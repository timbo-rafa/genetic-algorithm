...
from graph import Graph, f_path_cost #
...
class GA():
  ...
  def split_path(self, path, divisor):
    rest = path.size % divisor
    spath = np.split(path[:-rest], divisor)
    spath[0] = np.append(spath[0], path[-rest:])
    spath[0] = np.insert(spath[0], 0, self.g.source)
    spath[-1] = np.append(spath[-1], self.g.source)
    return spath

    
  def fitness(self, chromosome):
    spath = self.split_path(chromosome.path, self.number_workers)
    ans = []
    for ar in spath:
      ans.append(self.pool.apply_async(f_path_cost, args=(self.g, ar)))
    s = 0
    for a in ans:
      s += a.get()
    return s
